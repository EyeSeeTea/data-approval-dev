SELECT dataset.name                                      AS dataset,
         ...
         _periodstructure.<%= periodTypeColumn %>                              AS period,
  ...
  FROM ((SELECT distinct datavalue.periodid,
              ...
              (select workflowid from dataset where uid  ~ ('^' || replace('${dataSets}', '-', '|') || '$') )as workflowid,
               ...
               not in (select dataelementid from dataelement where name='<%= submissionDataElementName %>') 
...
(select
     CASE WHEN ( select dv2.lastupdated  from datavalue dv2 where 
            dv2.dataelementid in (select de.dataelementid from dataelement de where de.name like '<%= approvalDataElementName %>') 
  ...
  GROUP BY dva.periodid, dva.sourceid, dva.attributeoptioncomboid, dva.dataelementid
                ...
                (select dv1.value from datavalue dv1 where dv1.dataelementid = (select dataelementid from dataelement where name='<%= submissionDataElementName %>') and dv1.sourceid = datavalue.sourceid and dv1.periodid=datavalue.periodid) as lastdatesubmited,
                (select dv1.value from datavalue dv1 where dv1.dataelementid = (select dataelementid from dataelement where name='<%= approvalDataElementName %>') and dv1.sourceid = datavalue.sourceid and dv1.periodid=datavalue.periodid) as lastdateofapproval
         FROM _view_<%= viewTableName %> datavalue
         /** TODO: Filter by DEs, remove totals **/       
WHERE  periodid in (select periodid from period where periodtypeid = (select periodtypeid from periodtype where name='<%= periodTypeName %>'))
       and periodid in (select periodid from _periodstructure where _periodstructure.iso ~ ('^' || replace('${periods}', '-', '|') || '$'))
      AND sourceid in ( select dss.sourceid from  datasetsource dss  where  dss.datasetid = (select datasetid from dataset where uid ~ ('^' || replace('${dataSets}', '-', '|') || '$') )) 
 and datavalue.dataelementid not in (select dataelementid from dataelement where  name='<%= submissionDataElementName %>')
